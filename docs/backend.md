...
**网关并发实测（2026-08）**：stepfun 官方对**在途长请求**的并发上限为 10，超限直接返回 429（`concurrency reached, limit: 10`）——注意短请求压测会严重高估上限（1 秒级请求快进快出，32 并发也堆不起在途数），预热这类长生成请求必须按 10 封顶，默认取 9；saurlax 有效并发仅 ≈2 且超发不报 429 而是排队挂起 20-60s，吞吐反而从 1.15 请求/秒跌到 0.2，已弃用。预热脚本默认按 `aigw×2 + aigw2×9`，服务端把网关 429 透传为 HTTP 429 供脚本等待补跑。
...
`.dev.vars` 同时配置 `AIGW_API_KEY` 与 `AIGW2_API_KEY` 时自动启用双通道：任务按各通道并发比例分流，默认 `aigw×2 + aigw2×9`（实测安全值，见上方压测结论），单 stepfun 通道全量约 1-2 小时。单网关配置则单池运行。偶发失败（网关限流/热重载窗口/网络抖动）自动补跑一轮，重跑幂等（已缓存直接命中并自增 hits）。
